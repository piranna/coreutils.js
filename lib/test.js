...
{
  return statSync(path).gid === posix.getegid()
}
function ownedEffectiveUserID(file)
{
  return statSync(path).uid === posix.geteuid()
}
...
{
  return statSync(path).mode & BIT_SETGID
}
function setUserIDbit(file)
{
  return statSync(path).mode & BIT_SETUID
}
...
{
  return statSync(path).mode & BIT_STICKY
}
...
{
  return statSync(file1).mtime.getTime() > statSync(file2).mtime.getTime()
}
...
{
  file1 = statSync(file1)
  file2 = statSync(file2)
...
function test()
{
  const tokens = arguments[0] instanceof Array
               ? arguments[0]
  ...
  function decode()
  {
    ...
    while(tokens.length)
    {
      ...
      switch(token)
      {
        ...
        case ')':
          if(--parenthesis<0)
            throw new SyntaxError ('Unbalanced parenthesis underflow')
    ...
    if(parenthesis != 0) throw new SyntaxError('Unbalanced parenthesis')
