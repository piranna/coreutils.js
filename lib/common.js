...
function isBuffer (chunk) {
  // it seems chunks emitted from a readable are considered to not be buffers by
  // the browserify-buffer module mainly because `instanceof chunk !== Buffer`,
  // although `chunk` is actually a `Buffer`. However, these Buffers have an
  // `.offset` and `.get`, and numerous `.read` methods, so if we find these
  // we'll assume it's a buffer
...
//      if(.joiner) this.push(.joiner)

    if(typeof chunk === 'string')
      data = chunk
    else if(isBuffer(chunk) || chunk instanceof String)
      data = chunk.toString()
