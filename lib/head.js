...
function verbose(stream, args)
{
  stream = stream.pipe(OrderedReadStreams())

  let quiet = args['quiet'] || args['silent'] || args.q
  if((args['verbose'] || args.v || !quiet) !== false)
    stream.on('stream:start', function(stream)
    {
      const path = stream.path || ''
      this.push(`==> {stream.path} <==\n`)
    })
    .on('stream:end', function(stream)
    {
      this.push('\n')
    })
...
function head()
{
  ...
  const delayedLoading = args._delayedLoading
  if(!delayedLoading)
    files = files.map(getStream.bind(null, args))

  return Readable({
    objectMode: true,
    read() {
      if(!files.length) return this.push(null)

      return delayedLoading ? getStream(args, files.shift()) : files.shift()
    }
  })
}

head.Head    = Head
head.verbose = verbose
